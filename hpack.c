...
//	if ((str = recallocarray(data, data_len, data_len + 1, 1)) == NULL) {
	if ((str = realloc(data, sizeof(*data) * (data_len + 1))) == NULL) {
...
//	if ((ptr = recallocarray(buf->data, buf->size, newsize, 1)) == NULL)
	if ((ptr = realloc(buf->data, sizeof(*buf->data) * (newsize + 1))) == NULL)
		return (-1);
...
//		    buf->size, buf->wpos, 1)) == NULL) {
			if ((data = realloc(buf->data, sizeof(*buf->data) * (buf->wpos + 1))) == NULL) {
			hbuf_free(buf);
			return (NULL);
		}
		buf->data = data;
