...
int http2_write(struct connection *conn, char** buf, int len);
...
log_dbg(5,"0 ");
		for (int i = 0; http_status[i].status != NULL; i++) {
			log_dbg(5,"- code %i coded %i", code, http_status[i].coded);
			  if (code == http_status[i].coded) {
					  log_dbg(5,"1 bingo ");
				    return http_status[i].status;
				}
		}
...
{
		log_dbg(5, "%s: strm %p h2_stream_state %i f_typ %i", __func__, strm, h2_ss, f_typ);
...
process_frame(struct connection *conn, struct h2_frame frm, unsigned char* data)
{
	  struct stream *strm = NULL;
		...
		log_dbg(5, "%s: (H2 frame) f_sid %i f_typ %i f_flg %i f_len %i", __func__, frm.f_sid, frm.f_typ, frm.f_flg, frm.f_len);
...
http2_error(struct connection *conn, int code, uint32_t f_sid, char *buf, int len)
{
	  ...
	  int offset = 0;
		...
		// add headers
		if (hpack_header_add(conn->hpack_hbe, ":status", code_msg(code), 0) == NULL)
				goto err;
...
int
http_error(enum http_protocol http_protocol, int code, char *buf, int len)
{
		const char* status;
		const char* body;
		...
		wlen = snprintf(buf, len, "%s %s\r\n", (http_protocol) ? http_protocols[http_protocol] : "HTTP/1.0", status);
